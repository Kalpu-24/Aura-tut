...
void UAuraAttributeSet::HandleDebuff(const FEffectPropertiesEnhanced& Props)
{
	...
	const FGameplayTag DebuffTag = DamageTypeToDebuffTagMap[DamageTypeTag];
	InheritedTags.Added.AddTag(DebuffTag);


	if (DebuffTag.MatchesTagExact(TAG_Debuff_Stun))
	{
		InheritedTags.Added.AddTag(TAG_Player_Block_CursorTrace);
		InheritedTags.Added.AddTag(TAG_Player_Block_InputHeld);
		InheritedTags.Added.AddTag(TAG_Player_Block_InputReleased);
		InheritedTags.Added.AddTag(TAG_Player_Block_InputPressed);
	}
...
void UAuraAttributeSet::HandleIncomingDamage(const FEffectPropertiesEnhanced& Props)
{
	...
	if (LocalIncomingDamage > 0.f)
	{
		...
		else
		{
			...
			{
				Props.TargetProperties->Character->LaunchCharacter(KnockbackVector, true, true);
			}
		}
...
void UAuraAttributeSet::HandleIncomingXP(const FEffectPropertiesEnhanced& Props)
{
	...
	if (Props.SourceProperties->Character->Implements<UPlayerInterface>() && Props.SourceProperties->Character->Implements<UCombatInterface>())
	{
		...
		if (NumOfLevelUps > 0)
		{
			...
			{
				SpellPointsReward += IPlayerInterface::Execute_GetSpellPointsReward(Props.SourceProperties->Character, CurrentLevel + i);
				AttributePointsReward += IPlayerInterface::Execute_GetAttributePointsReward(Props.SourceProperties->Character, CurrentLevel + i);
			}
